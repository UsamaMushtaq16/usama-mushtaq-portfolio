...
const Hero = ({ mainData }: HeroProps) => {
    ...
    return (
        <section id='home' className={`${theme === 'dark' && "bg-grey-900"} relative min-h-screen w-full mx-auto overflow-hidden`}>
            ...
            <div className="opacity-50 md:opacity-40 absolute -z-10 min-h-screen h-lvh w-full">
                <Image
                    src="/herobgc.jpg"
                    layout="fill"
                    objectFit="cover"
            ...
            <div className="py-16 lg:pt-48 flex flex-col-reverse lg:flex-row justify-around gap-10 lg:gap-0">
                ...
                <div className="flex flex-col gap-4 md:gap-6 text-left lg:w-1/2 2xl:w-1/3 mx-4 md:mx-6 xl:mx-0">
                    ...
                    <ScrollLink
                        className="w-fit text-sm md:text-base mt-2 py-2 px-4 cursor-pointer flex items-center gap-1 rounded-md bg-violet-600 hover:bg-violet-700 dark:bg-violet-700 hover:dark:bg-violet-800 transition-colors group text-white"
                        to={'about'}
                        offset={-60}
                        smooth={true}
                        duration={500}
