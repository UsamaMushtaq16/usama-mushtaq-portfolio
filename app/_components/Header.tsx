...
export default function Header({ logo }: { logo: string }) {
    ...
    return (
        <header className={`backdrop-filter backdrop-blur-lg ${scroll ? 'border-b bg-white bg-opacity-40' : 'border-b-0'} dark:bg-grey-900 dark:bg-opacity-40 border-gray-200 dark:border-b-0 z-30 min-w-full flex flex-col fixed`}>
            ...
            <nav className='lg:w-11/12 2xl:w-4/5 w-full md:px-6 2xl:px-0 mx-auto py-4 sm:flex items-center justify-between max-sm:hidden'> 
                <Link href={'/'} className='2xl:ml-6 flex items-center gap-2'>
                    <Image className={'nav-logo-image'} src={logoIcon} alt="Logo Gif" /><span className='text-lg font-bold'>{logo}</span>
                </Link>
                <ul className='flex items-center gap-8'>
                    {navs.map((e, i) => (
                        <li key={i}>
                            <ScrollLink
                                className='hover:hover:text-violet-700 hover:dark:text-violet-500 transition-colors capitalize cursor-pointer text-sm font-medium border-styling'
                                to={e}
                                offset={-60}
                                smooth={true}
                                duration={500}
